...
class RetailerShellLayout extends StatefulWidget {
  final StatefulNavigationShell navigationShell;

  const RetailerShellLayout({
    super.key,
    required this.navigationShell,
  });

  @override
  State<RetailerShellLayout> createState() => _RetailerShellLayoutState();
}

class _RetailerShellLayoutState extends State<RetailerShellLayout> {
      ...
      body: Row(
        children: [
          NavigationRail(
            ...
            backgroundColor: AppColors.white,
            selectedIconTheme: const IconThemeData(color: AppColors.roleRetailer),
            unselectedIconTheme: const IconThemeData(color: AppColors.neutral500),
            selectedLabelTextStyle: const TextStyle(color: AppColors.roleRetailer, fontSize: 12),
            unselectedLabelTextStyle: const TextStyle(color: AppColors.neutral500, fontSize: 12),
            useIndicator: true,
            indicatorColor: AppColors.roleRetailerLight,
            leading: const Padding(
              padding: EdgeInsets.symmetric(vertical: 16.0),
              child: Icon(LucideIcons.store, color: AppColors.roleRetailer, size: 32),
            ),
            destinations: const [
              NavigationRailDestination(icon: Icon(LucideIcons.layoutDashboard), label: Text('Overview')),
              NavigationRailDestination(icon: Icon(LucideIcons.package), label: Text('Inventory')),
              NavigationRailDestination(icon: Icon(LucideIcons.messageSquare), label: Text('Inquiries')),
              ...
              NavigationRailDestination(icon: Icon(LucideIcons.shoppingBag), label: Text('Orders')),
            ],
          ),
